...
async function handleFrame(msg: Extract<DepthWorkerRequest, { type: "frame" }>): Promise<void> {
  ...
  const { tensor: input, region } = rgbaToInputTensor(msg.rgb, msg.width, msg.height, DEPTH_INPUT_SIZE);
  ...
  console.log(
    `[worker] preprocess=${region.mode} src=${region.srcWidth}x${region.srcHeight}@(${region.srcX},${region.srcY}) dst=${region.dstWidth}x${region.dstHeight}`,
  );
  console.log(`[worker] input tensor dims=1x3x${DEPTH_INPUT_SIZE}x${DEPTH_INPUT_SIZE}`);
  console.log(`[worker] output tensor dims=${output.dims.join("x")}`);
  ...
  post({
    ...
    region: {
      srcX: region.srcX,
      srcY: region.srcY,
      srcWidth: region.srcWidth,
      srcHeight: region.srcHeight,
    },
  });
